...
public class MntcUserMinTests
{
    ...
    private static async Task TestShouldPassClaims(List<Claim> claims)
    {
        ...
        var authHandler = new MntcUserMinimumAuthenticator.AuthHandler();
        ...
        var resourceFilter = new MntcUserMinimumAuthenticator.ResourceFilter();

        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        var actionFilter = new MntcUserMinimumAuthenticator.ActionFilter();
