...
public class CustomerMinimumAuthenticatorTests(ITestOutputHelper _output)
{
    ...
    public async Task ShouldReturnTrue_WhenUserIsAuthorizedAsync()
    {
        List<TestData> dataLists = [
            ...
            new TestData([IdTeamClaims.MAINTENANCE_TEAM,],4  ),    //Higher Team
        ];
    ...
    private static async Task TestShouldPassClaims(List<Claim> claims, int? requiredPosition)
    {
        ...
        var authHandler = new CustomerMinimumAuthenticator.AuthHandler();
        ...
        CustomerMinimumAuthenticator.ResourceFilter resourceFilter = requiredPosition is null
            ? new()
            : new(requiredPosition.Value);


        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        CustomerMinimumAuthenticator.ActionFilter actionFilter = requiredPosition is null
            ? new()
            : new(requiredPosition.Value);
    ...
    public async Task ShouldReturn_Forbid_WhenUserIsAuthorizedAsync()
    {
        ...
        List<TestData> datasLists = [
            new TestData([IdTeamClaims.CUSTOMER_TEAM, IdTeamClaims.TEAM_POSITION(-5)], null),
            new TestData([IdTeamClaims.CUSTOMER_TEAM, IdTeamClaims.TEAM_POSITION(10)], 12),
            new TestData([IdTeamClaims.CUSTOMER_TEAM, IdTeamClaims.TEAM_POSITION(4)],5  ),
    ...
    public async Task AuthHandler_IsAuthorized_ShouldReturnFalse_WhenUserIsNotAuthenticatedAsync()
    {
        ...
        var resourceExecutingContext = ContextProvider.GetResourceExecutingContext(httpContext);
        var resourceFilterNext = new Mock<ResourceExecutionDelegate>();
        var resourceFilter = new CustomerMinimumAuthenticator.ResourceFilter();

        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        var actionFilter = new CustomerMinimumAuthenticator.ActionFilter();
