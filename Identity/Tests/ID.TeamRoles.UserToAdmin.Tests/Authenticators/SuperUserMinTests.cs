...
public class SuperUserMinTests
{
    [Fact]
    ...
    private static async Task TestShouldPassClaims(List<Claim> claims)
    {
        ...
        var authHandler = new SuperUserMinimumAuthenticator.AuthHandler();
        ...
        var resourceFilter = new SuperUserMinimumAuthenticator.ResourceFilter();

        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        var actionFilter = new SuperUserMinimumAuthenticator.ActionFilter();
