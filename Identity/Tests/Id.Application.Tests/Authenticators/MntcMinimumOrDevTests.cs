...
public class MntcMinimumOrDevAuthenticatorTests(ITestOutputHelper _output)
{
    ...
    private async Task TestShouldPassClaims(List<Claim> claims, int? requiredPosition, bool isDevMode)
    {
        ...
        var authHandler = new MntcMinimumOrDevAuthenticator.AuthHandler();
        ...
        MntcMinimumOrDevAuthenticator.ResourceFilter resourceFilter = requiredPosition is null
            ? new()
            : new(requiredPosition.Value);


        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        MntcMinimumOrDevAuthenticator.ActionFilter actionFilter = requiredPosition is null
            ? new()
            : new(requiredPosition.Value);
    ...
    public async Task AuthHandler_IsAuthorized_ShouldReturnFalse_WhenUserIsNotAuthenticatedAsync()
    {
        ...
        var resourceExecutingContext = ContextProvider.GetResourceExecutingContext(httpContext);
        var resourceFilterNext = new Mock<ResourceExecutionDelegate>();
        var resourceFilter = new MntcMinimumOrDevAuthenticator.ResourceFilter();

        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        var actionFilter = new MntcMinimumOrDevAuthenticator.ActionFilter();
