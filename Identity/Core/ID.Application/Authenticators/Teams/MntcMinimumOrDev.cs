...
public static class MntcMinimumOrDevAuthenticator
{
    /// <summary>
    /// Only Mntc members with position Greater or equal to <paramref name="level"/>, or Higher team shall pass!
    /// </summary>
    protected internal class AuthHandler : IAuthPositionHandler
    {
        ...
        public bool IsAuthorized(HttpContext context, int? level = -1)
        {
            ...
            var isInMntcTeam = context.IsInMntcTeam();
            var isHigherThanCurrentUserTeam = context.IsInSuperTeam();

            var teamPosition = context.TeamPosition();


            return isDev
                ||
                isHigherThanCurrentUserTeam
                ||
                isInMntcTeam && teamPosition >= level;
        }
...
public static class MntcMinimumOrDevAuthenticatorPolicySetupExtensions
{
    public static IServiceCollection AddMntcMinimumOrDevAuthenticatorPolicy(this IServiceCollection services)
    {
        services.AddSingleton<IAuthorizationHandler, MntcMinimumOrDevAuthenticator.Policy.Handler>();
        return services.AddAuthorization(options =>
              options.AddPolicy(MntcMinimumOrDevAuthenticator.Policy.NAME, policy =>
                  policy
                    //.RequireAuthenticatedUser() //This is handled in the Handler
                    .Requirements.Add(new MntcMinimumOrDevAuthenticator.Policy.Requirement(null)))
        );
    }
}//Cls
