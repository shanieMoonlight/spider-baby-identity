...
public class ResendEmailConfPrincipalCmdHandlerTests
{
    ...
    public async Task Should_Return_SuccessWithMessage_WhenAlreadyConfirmed()
    {
        ...
        var cmd = new ResendEmailConfirmationPrincipalCmd
        {
            PrincipalUser = dbUser,
            PrincipalUserId = dbUser.Id
        };
        cmd.PrincipalUser.Email = dbUser.Email;

        ResendEmailConfirmationPrincipalHandler handler = new(_mockConfBus.Object);
    ...
    public async Task Should_PublishEventWhenUserFound()
    {
        ...
        result.Info.ShouldBe(IDMsgs.Info.Email.EMAIL_CONFIRMATION_SENT(dbUser.Email));
