...
internal static class AuthRequestHelpersExtensions
{
    ...
    /// For use in Validation Tests
    /// </summary>
    internal static TRequest SetAuthenticated_MNTC<TRequest>(this TRequest request, IEnumerable<Claim> claims) where TRequest : APrincipalInfoRequest
    {
        request.IsAuthenticated = true;
        request.IsCustomer = false;
        request.IsMntc = true;
        request.IsSuper = false;
        request.Principal = new ClaimsPrincipal(new ClaimsIdentity(claims, "TestAuth"));
        request.PrincipalEmail = "a@b.cd";
        request.PrincipalTeamId = Guid.NewGuid();
        request.PrincipalTeamPosition = 500;
        request.PrincipalUserId = Guid.NewGuid();
    ...
    /// For use in Validation Tests
    /// </summary>
    internal static TRequest SetAuthenticated_SUPER<TRequest>(this TRequest request, IEnumerable<Claim> claims) where TRequest : APrincipalInfoRequest
    {
        request.IsAuthenticated = true;
        request.IsCustomer = false;
        request.IsMntc = false;
    ...
    /// For use in Validation Tests
    /// </summary>
    internal static TRequest SetAuthenticated_CUSTOMER<TRequest>(this TRequest request, IEnumerable<Claim> claims) where TRequest : APrincipalInfoRequest
    {
        request.IsAuthenticated = true;
        request.IsCustomer = true;
        request.IsMntc = false;
