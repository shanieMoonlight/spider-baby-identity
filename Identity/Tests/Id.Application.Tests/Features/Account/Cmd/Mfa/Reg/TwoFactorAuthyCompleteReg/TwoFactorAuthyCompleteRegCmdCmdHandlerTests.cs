...
public class TwoFactorAuthyCompleteRegCmdCmdHandlerTests
{
    private readonly Mock<IAuthenticatorAppService> _googleAuthMock;
    private readonly TwoFactorAuthyCompleteCmdHandler _handler;

    public TwoFactorAuthyCompleteRegCmdCmdHandlerTests()
    {
        _googleAuthMock = new Mock<IAuthenticatorAppService>();
        _handler = new TwoFactorAuthyCompleteCmdHandler(_googleAuthMock.Object);
    }
    ...
    public async Task Handle_ShouldReturnSuccess_WhenEnableAsyncSucceeds()
    {
        ...
        var request = new TwoFactorAuthyCompleteRegCmd("valid-code") { PrincipalUser = user };
    ...
    public async Task Handle_ShouldReturnFailure_WhenEnableAsyncFails()
    {
        ...
        var team = TeamDataFactory.Create();
        var user = AppUserDataFactory.Create(team: team);
        var request = new TwoFactorAuthyCompleteRegCmd("invalid-code") { PrincipalUser = user };
