...
{
    [Fact]
    public async Task AuthHandler_IsAuthorized_ShouldReturnTrue_WhenUserIsInSuperTeamAndUserRoleAsync()
    {
        // Arrange
        ...
        var authHandler = new SuperUserAuthenticator.AuthHandler();
        ...
        var resourceFilter = new SuperUserAuthenticator.ResourceFilter();

        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        var actionFilter = new SuperUserAuthenticator.ActionFilter();
