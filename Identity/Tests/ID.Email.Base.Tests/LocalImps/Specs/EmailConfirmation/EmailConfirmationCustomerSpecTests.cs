...
public class EmailConfirmationCustomerSpecTests
{
    [Fact]
    public async Task BuildAsync_ShouldCallGenerateTemplateWithCallback_AndReturnEmailDetails()
    {
        // Arrange
        var toName = "Customer";
        var toAddress = "customer@example.com";
        var callbackUrl = "https://example.com/confirm?token=cust";
        ...
        helpersMock.Verify(h => h.GenerateTemplateWithCallback(
            ...
            toAddress,
            callbackUrl,
            It.Is<string>(s => s.EndsWith("EmailConfirmation\\IdEmailConfirmationCustomer.html")),
