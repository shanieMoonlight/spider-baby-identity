...
public class CustomerAuthenticatorTests(ITestOutputHelper _output)
{
    ...
    public async Task ShouldReturnTrue_WhenUserIsAuthorizedAsync()
    {
        List<TestData> dataLists = [
            new TestData([IdTeamClaims.CUSTOMER_TEAM, IdTeamClaims.TEAM_POSITION(-5)], null), //Null means don't check posiition
            new TestData([IdTeamClaims.CUSTOMER_TEAM, IdTeamClaims.TEAM_POSITION(0)], 0),
            new TestData([IdTeamClaims.CUSTOMER_TEAM, IdTeamClaims.TEAM_POSITION(11)], 11),
            new TestData([IdTeamClaims.CUSTOMER_TEAM, IdTeamClaims.TEAM_POSITION(555)],555  ),
        ];
    ...
    private static async Task TestShouldPassClaims(List<Claim> claims, int? requiredPosition)
    {
        ...
        var authHandler = new CustomerAuthenticator.AuthHandler();
        ...
        CustomerAuthenticator.ResourceFilter resourceFilter = requiredPosition is null
            ? new()
            : new(requiredPosition.Value);


        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        CustomerAuthenticator.ActionFilter actionFilter = requiredPosition is null
            ? new()
            : new(requiredPosition.Value);
    ...
    public async Task ShouldReturn_Forbid_WhenUserIsAuthorizedAsync()
    {
        ...
        List<TestData> datasLists = [
            new TestData([IdTeamClaims.CUSTOMER_TEAM, IdTeamClaims.TEAM_POSITION(10)], 12),//Wrong Position
            new TestData([IdTeamClaims.CUSTOMER_TEAM, IdTeamClaims.TEAM_POSITION(4)],5  ),//Wrong Position
            new TestData([IdTeamClaims.SUPER_TEAM, IdTeamClaims.TEAM_POSITION(-1)], null),//Wrong Team - Right position
            new TestData([IdTeamClaims.SUPER_TEAM, IdTeamClaims.TEAM_POSITION(11)], 11),//Wrong Team - Right position
            ...
            new TestData([IdTeamClaims.MAINTENANCE_TEAM, IdTeamClaims.TEAM_POSITION(-1)], null),  //Wrong Team - Right position
            new TestData([IdTeamClaims.MAINTENANCE_TEAM, IdTeamClaims.TEAM_POSITION(11)], 11),   //Wrong Team - Right position
            new TestData([IdTeamClaims.MAINTENANCE_TEAM, IdTeamClaims.TEAM_POSITION(4)],4  ),    //Wrong Team - Right position
            new TestData([IdTeamClaims.MAINTENANCE_TEAM, IdTeamClaims.TEAM_POSITION(5)],5  ),    //Wrong Team - Right position
            new TestData([IdTeamClaims.MAINTENANCE_TEAM,],4  ),    //Wrong Team - 
        ];
    ...
    public async Task AuthHandler_IsAuthorized_ShouldReturnFalse_WhenUserIsNotAuthenticatedAsync()
    {
        ...
        var resourceExecutingContext = ContextProvider.GetResourceExecutingContext(httpContext);
        var resourceFilterNext = new Mock<ResourceExecutionDelegate>();
        var resourceFilter = new CustomerAuthenticator.ResourceFilter();

        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        var actionFilter = new CustomerAuthenticator.ActionFilter();
