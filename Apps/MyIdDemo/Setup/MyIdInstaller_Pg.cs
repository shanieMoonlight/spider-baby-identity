...
/// </summary>
public class MyIdInstaller_Pg : IServiceInstaller
{
    /// <summary>
    ...
    public WebApplicationBuilder Install(WebApplicationBuilder builder, StartupData startupData)
    {
        ...
        builder.Services.AddMyId_Postgres(
           config =>
           {
               config.ApplicationName = startupData.APP_NAME;
               config.ConnectionString = startupData.ConnectionStringsSection.GetPostgresDb();
           ...
           }).AddMyIdAmazonOAuth(config =>
           {
               config.ClientId = startupData.OAuthSection.AmazonSection.GetClientId()!;
               config.ClientSecret = startupData.OAuthSection.AmazonSection.GetClientSecret()!;
           })
...
/// Install Identity stuff
/// </summary>
public static class MyIdInstallerPgExtensions
{
    ...
    /// <param name="builder">Application Builder</param>
    /// <param name="startupData">All the app config and settings</param>
    public static WebApplicationBuilder InstallMyId_Pg(this WebApplicationBuilder builder, StartupData startupData) =>
        new MyIdInstaller_Pg().Install(builder, startupData);
