...
public static class MntcMinimumAuthenticator
{
    ...
    protected internal class AuthHandler : IAuthPositionHandler
    {
        ...
        public bool IsAuthorized(HttpContext context, int? level = -1)
        {
            ...
            var isHigherThanCurrentUserTeam = isInSprTeam;

            var teamPosition = context.User.TeamPosition();

            return isHigherThanCurrentUserTeam || isInMntcTeam && teamPosition >= level;
        }
...
public static class MntcMinimumAuthenticatorPolicySetupExtensions
{
    public static IServiceCollection AddMntcMinimumAuthenticatorPolicy(this IServiceCollection services)
    {
        services.AddSingleton<IAuthorizationHandler, MntcMinimumAuthenticator.Policy.Handler>();
        return services.AddAuthorization(options =>
              options.AddPolicy(MntcMinimumAuthenticator.Policy.NAME, policy =>
                  policy
                    .RequireAuthenticatedUser()
                    .Requirements.Add(new MntcMinimumAuthenticator.Policy.Requirement(null)))
        );
    }
}//Cls
