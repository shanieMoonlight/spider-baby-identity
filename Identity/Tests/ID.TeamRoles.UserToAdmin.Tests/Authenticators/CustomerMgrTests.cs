...
public class CustomerMgrTests
{
    [Fact]
    public async Task AuthHandler_IsAuthorized_ShouldReturnTrue_WhenUserIsInCustomerTeamAndManagerRoleAsync()
    {
        // Arrange
        List<Claim> claims = [IdTeamClaims.CUSTOMER_TEAM, IdTeamRoleClaims.MANAGER];
        var httpContext = ContextProvider.GetHttpContext(claims, true);
        var authHandler = new CustomerManagerAuthenticator.AuthHandler();
        ...
        var resourceFilter = new CustomerManagerAuthenticator.ResourceFilter();

        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        var actionFilter = new CustomerManagerAuthenticator.ActionFilter();
