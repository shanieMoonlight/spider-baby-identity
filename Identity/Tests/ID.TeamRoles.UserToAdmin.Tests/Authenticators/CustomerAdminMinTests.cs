...
public class CustomerAdminMinTests
{
    [Fact]
    ...
    private static async Task TestShouldPassClaims(List<Claim> claims)
    {
        ...
        var authHandler = new CustomerAdminMinimumAuthenticator.AuthHandler();
        ...
        var resourceFilter = new CustomerAdminMinimumAuthenticator.ResourceFilter();

        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        var actionFilter = new CustomerAdminMinimumAuthenticator.ActionFilter();
