...
{
    [Fact]
    public async Task AuthHandler_IsAuthorized_ShouldReturnTrue_WhenUserIsInSuperTeamAndAdminRoleAsync()
    {
        // Arrange
        ...
        var authHandler = new SuperAdminAuthenticator.AuthHandler();
        ...
        var resourceFilter = new SuperAdminAuthenticator.ResourceFilter();

        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        var actionFilter = new SuperAdminAuthenticator.ActionFilter();
