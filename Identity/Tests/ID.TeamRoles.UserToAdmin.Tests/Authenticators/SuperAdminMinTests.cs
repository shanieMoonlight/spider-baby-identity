...
public class SuperAdminMinTests
{
    [Fact]
    ...
    private static async Task TestShouldPassClaims(List<Claim> claims)
    {
        ...
        var authHandler = new SuperAdminMinimumAuthenticator.AuthHandler();
        ...
        var resourceFilter = new SuperAdminMinimumAuthenticator.ResourceFilter();

        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        var actionFilter = new SuperAdminMinimumAuthenticator.ActionFilter();
