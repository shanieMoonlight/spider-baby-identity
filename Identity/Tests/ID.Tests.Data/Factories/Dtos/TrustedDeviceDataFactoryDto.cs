...
public static class TrustedDeviceDtoDataFactory
{

    public static List<TrustedDeviceDto> CreateMany(int count = 20) => 
        [.. IdGenerator.GetGuidIdsList(count).Select(id => Create(id))];

    //- - - - - - - - - - - - - //

    public static TrustedDeviceDto Create(
        Guid? id = null,
		Guid? userId = null,
        ...
        ) 
    {
       ...
       return ConstructorInvoker.CreateNoParamsInstance<TrustedDeviceDto>(paramaters);
    }

    //--------------------------// 

   public static TrustedDeviceDto Update(
        ...
        ) 
    {
        ...
        List<PropertyAssignment> propertAssignments = [];
            ...
            if (userId is not null || user?.Id is not null)
       ...
       return PrivatePropertyUpdater.UpdateProperties(trustedDeviceDto, [.. propertAssignments]);
    }
