...
public class CustomerLeaderMinimumAuthenticatorTests(ITestOutputHelper _output)
{
    ...
    private static async Task TestShouldPassClaims(List<Claim> claims)
    {
        ...
        var authHandler = new CustomerLeaderMinimumAuthenticator.AuthHandler();
        ...
        CustomerLeaderMinimumAuthenticator.ResourceFilter resourceFilter = new();


        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        CustomerLeaderMinimumAuthenticator.ActionFilter actionFilter = new();
    ...
    public async Task ShouldReturn_Forbid_WhenUserIsAuthorizedAsync()
    {
        List<List<Claim>> claimsLists = [
            ...
            //[IdTeamClaims.SUPER_TEAM, IdTeamClaims.LEADER], //Wrong Team
            //[IdTeamClaims.MAINTENANCE_TEAM, IdTeamClaims.LEADER], //Wrong Team
    ...
    public async Task AuthHandler_IsAuthorized_ShouldReturnFalse_WhenUserIsNotAuthenticatedAsync()
    {
        ...
        var resourceExecutingContext = ContextProvider.GetResourceExecutingContext(httpContext);
        var resourceFilterNext = new Mock<ResourceExecutionDelegate>();
        var resourceFilter = new CustomerLeaderMinimumAuthenticator.ResourceFilter();

        var actionContext = ContextProvider.GetActionExecutingContext(httpContext);
        var actionFilter = new CustomerLeaderMinimumAuthenticator.ActionFilter();
